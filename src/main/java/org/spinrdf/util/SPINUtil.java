...
import org.spinrdf.model.CommandWithWhere;
...
public class SPINUtil {
     ...
     */
    public static void bindThis(QueryExecutionBuilder qexecBuilder, RDFNode value) {
        if(value != null) {
    ...
    public static String getQueryString(RDFNode node, boolean usePrefixes) {
        if(node.isLiteral()) {
            ...
            org.spinrdf.model.Command spinCommand = SPINFactory.asCommand(resource);
            if(spinCommand != null) {
                if(usePrefixes) {
