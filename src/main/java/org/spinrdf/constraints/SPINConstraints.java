...
import org.spinrdf.system.SPINImports;
import org.spinrdf.system.SPINLabels;
import org.spinrdf.util.CommandWrapper;
import org.spinrdf.util.JenaUtil;
import org.spinrdf.util.PropertyPathsGetter;
...
public class SPINConstraints {
    ...
    private static void addTemplateCallResults(List<ConstraintViolation> results,
        ...
        if(template != null && template.getBody() instanceof org.spinrdf.model.Query) {
            org.spinrdf.model.Query spinQuery = (org.spinrdf.model.Query) template.getBody();
        ...
        if(thisUnbound || SPINUtil.isRootClass(cls) || model.contains(null, RDF.type, cls)) {
            ...
            if(thisDeep && !thisUnbound) {
                ...
                while(it.hasNext()) {
                    ...
                    QueryExecution qexec = ARQFactory.get().createQueryExecution(arq, model, arqBindings).build();
