...
public class ARQFactory {
     ...
     * @param qexecBuilder  the QueryExecutionBuilder to modify
     */
    protected void adjustQueryExecution(QueryExecutionBuilder qexecBuilder) {
    }
     ...
     */
    public QueryExecutionBuilder createQueryExecution(Query query, Model model) {
     ...
     * @return a QueryExecution
     */
    public QueryExecutionBuilder createQueryExecution(Query query, Model model, QuerySolution initialBinding) {
        Dataset dataset = getDataset(model);
        if(dataset != null) {
            return createQueryExecution(query, dataset, initialBinding);
        }
        else {
            QueryExecutionBuilder qexecBuilder = QueryExecution.model(model).query(query).substitution(initialBinding);
            adjustQueryExecution(qexecBuilder);
            return qexecBuilder;
        }
    }


    public QueryExecutionBuilder createQueryExecution(Query query, Dataset dataset) {
        return createQueryExecution(query, dataset, null);
    }


    public QueryExecutionBuilder createQueryExecution(Query query, Dataset dataset, QuerySolution initialBinding) {
        if(!query.getGraphURIs().isEmpty() || !query.getNamedGraphURIs().isEmpty()) {
            dataset = new FromDataset(dataset, query);
        }
        QueryExecutionBuilder qexecBuilder = QueryExecution.dataset(dataset).query(query).substitution(initialBinding);
        adjustQueryExecution(qexecBuilder);
        return qexecBuilder;
    }
     ...
     */
    public QueryExecutionHTTPBuilder createRemoteQueryExecution(Query query) {
        List<String> graphURIs = query.getGraphURIs();
        return createRemoteQueryExecution(query, graphURIs);
    }


    public QueryExecutionHTTPBuilder createRemoteQueryExecution(Query query, List<String> graphURIs) {
...
//	}
    ...
    public QueryExecutionHTTPBuilder createRemoteQueryExecution(String service, Query query, List<String> defaultGraphURIs,
                                                      List<String> namedGraphURIs, String user, String password) {
        HttpClient httpClient = buildHttpClient(service, user, password);
        QueryExecutionHTTPBuilder qexecBuilder = QueryExecution.service(service)
                .query(query)
                .httpClient(httpClient);
        defaultGraphURIs.forEach(qexecBuilder::addDefaultGraphURI);
        namedGraphURIs.forEach(qexecBuilder::addNamedGraphURI);
        return qexecBuilder;
    }
    ...
    public static HttpClient buildHttpClient(String serviceURI, String user, String password) {
        HttpClient result;
        if ( user == null ) {
            result = HttpEnv.getHttpClient(serviceURI, null);
        } else {
