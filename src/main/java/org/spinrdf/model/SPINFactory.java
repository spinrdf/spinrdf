...
public class SPINFactory {
    ...
    public static Element asElement(Resource resource) {
        ...
        else if(resource.canAs(Lateral.class)) {
            return resource.as(Lateral.class);
        }
    ...
    /**
     * Creates a new Lateral as a blank node in a given Model.
     * @param model  the Model to create the LATERAL in
     * @param elements  the elements of the LATERAL
     * @return a new Lateral
     */
    public static Lateral createLateral(Model model, ElementList elements) {
        Lateral lateral = model.createResource(SP.Lateral).as(Lateral.class);
        lateral.addProperty(SP.elements, elements);
        return lateral;
    }
